...
int main()
{
    ...
    string currentLine;
    string highestAlpha, lowestAlpha;

    getline(inputFile, currentLine);
    highestAlpha = lowestAlpha = currentLine;

    for (int i = 1; i < studentNum; ++i) {
        getline(inputFile, currentLine);

        //TODO figure out how to parse out the first name from currentLine
        
        //TODO check if currentLine's first name is higher than highestAlpha. if it is, assign its value to highestAlpha

        //TODO check if currentLine's first name is lower than lowestAlpha. if it is, assign its value to lowestAlpha
        
    }

    cout << highestAlpha << endl;
    cout << lowestAlpha << endl;
    /*inputFile >> name;
    getline(cin, name);

    int num;
    for (num = MIN_STUDENTS; num <= 2; num++)
        cout << num << ": " << name << endl;    
    */
}
