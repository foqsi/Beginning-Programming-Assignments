/*
One of your professors has asked you to write a program to grade her final exams, which consist of only 20 multiple-choice questions. Each question has one of four possible answers: A, B, C, D. The file CorrectAnswers.txt contains the correct answers for all of the questions, which each answer written on a separate line. The first line contains the answer to the first question, the second line contains the answer to the second question, and so forth.

Write a program that reads the contents of the CorrectAnswers.txt file int a char array, then reads the contents of another file, containing a student's answers, into a second char array. The program should determine the number of questions that the student missed, then display the following:
